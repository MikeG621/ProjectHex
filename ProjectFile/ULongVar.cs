...
namespace Idmr.ProjectHex
{
	public partial class ProjectFile
	{
		...
		{
			ulong _minValue = 0;
			ulong _maxValue = UInt64.MaxValue;
			...
			/// <exception cref="ArgumentOutOfRangeException"><i>minValue</i>, <i>maxValue</i> or <i>defaultValue</i> fall outside the range of <see cref="UInt64"/>.</exception>
			/// <remarks><see cref="RawValue"/> initializes to <b>0</b>.<br>
			/// If <i>minValue</i> or <i>maxValue</i> are empty or <b>null</b>, they default to the limits of <see cref="UInt64"/>.</remarks>
			public ULongVar(VarCollection parent, string minValue, string maxValue, string defaultValue) : base(parent, defaultValue)
			{
				if (minValue != null && minValue != "") _minValue = UInt64.Parse(minValue);
				if (maxValue != null && maxValue != "") _maxValue = UInt64.Parse(maxValue);
				UInt64.Parse(defaultValue);
				_type = VarType.ULong;
			}
			...
			/// <exception cref="OverflowException"><see cref="RawValue"/> does not fall between <b>0</b> and <see cref="UInt64.MaxValue"/>.</exception>
			public ulong Value
			{
			...
			/// <remarks>Defaults to <see cref="UInt64.MinValue"/>.<br/>
			...
			/// <exception cref="InvalidOperationException">Attribute is controlled by parent</exception>
			/// <remarks>Defaults to <see cref="UInt64.MaxValue"/>.<br/>
			/// If part of an array, gets the parent's attribute. Attempting to set results in an exception.</remarks>
			...
			/// <remarks>Always returns <b>false</b> if part of an array.</remarks>
			public bool UseMaxValue { get { return (_maxValue != UInt64.MaxValue); } }
